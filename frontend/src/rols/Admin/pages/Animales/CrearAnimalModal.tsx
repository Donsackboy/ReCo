...
interface CrearAnimalModalProps {
  open: boolean;
  onClose: () => void;
  refugios: any[];
...
const CrearAnimalModal: React.FC<CrearAnimalModalProps> = ({ open, onClose, refugios, onCreate }) => {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <div style={{ background: '#fff', padding: 32, borderRadius: 16, minWidth: 350, maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 8px 32px rgba(67,160,71,0.12)', position: 'relative' }}>
        <h2 style={{ color: '#43a047', fontWeight: 700, marginBottom: 18, textAlign: 'center', fontSize: 28 }}>Crear Animal</h2>
        <form onSubmit={handleSubmit} style={{ display: 'grid', gap: 18 }}>
          ...
          <div>
            <label htmlFor="refugio" style={{ fontWeight: 500, marginBottom: 4, display: 'block' }}>Refugio</label>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginTop: 8 }}>
            <button type="submit" className="btn-editar" style={{ background: '#43a047', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 28px', fontWeight: 700 }}>Crear animal</button>
...
export default CrearAnimalModal;
