...
export async function createVacunaAdmin(
  token: string,
  animalId: number,
  data: any
): Promise<any> {
...
export async function getListasVacunasEspecie(token: string): Promise<any[]> {
  ...
  if (!response.ok)
    throw new Error("Error al obtener listas de vacunas por especie");
  return response.json();
}

export async function createListaVacunasEspecie(
  token: string,
  data: any
): Promise<any> {
  const response = await fetch(`${API_BASE}/listas-vacunas-especie/`, {
  ...
  if (!response.ok)
    throw new Error("Error al crear lista de vacunas por especie");
  return response.json();
}
// CRUD para especies
